package com.vpsreactnativeimagepicker
...
class VpsReactNativeImagePickerModule(reactContext: ReactApplicationContext) :
...
var style = PictureSelectorStyle()
            ...
            if (requestCode == CAMERA_REQUEST) {
                if (resultCode == Activity.RESULT_OK) {
                    if (currentPromise != null) {
                        startPictureSelector(activity)
                    }
                } else if (resultCode == Activity.RESULT_CANCELED) {
                    currentPromise?.reject("PICKER_CANCELLED", "User has canceled", null)
                    currentPromise = null
                }
            }
        }
    ...
    @ReactMethod
    fun openCamera(options: ReadableMap?, promise: Promise): Unit {
        ...
        if (currentActivity != null) {
            if (currentActivity.checkSelfPermission(android.Manifest.permission.CAMERA) != PackageManager.PERMISSION_GRANTED) {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                    currentActivity.requestPermissions(arrayOf(android.Manifest.permission.CAMERA), MY_CAMERA_PERMISSION_CODE)
                }
            } else {
                startPictureSelector(currentActivity)
            }
        } else {
            currentPromise?.reject("ACTIVITY_NOT_FOUND", "Activity not available")
            currentPromise = null
        }
    }


    private fun startPictureSelector(activity: Activity) {
        PictureSelector.create(activity)
                ...
                .forResultActivity(object : OnResultCallbackListener<LocalMedia?> {
                    override fun onResult(result: ArrayList<LocalMedia?>?) {
                        ...
                        if (result?.size == 0) {
                            currentPromise?.resolve(localMedia)
                            return
                        }
    ...
    @ReactMethod
    fun openImagePicker(options: ReadableMap?, promise: Promise): Unit {
...
//        pictureStyle.pictureTitleBarBackgroundColor = Color.parseColor("#393a3e")
  ...
  companion object {
    const val NAME = "VpsReactNativeImagePicker"
  }
}
