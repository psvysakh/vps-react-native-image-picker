...
const { VpsReactNativeImagePicker } = NativeModules;
...
export const openImagePicker = (optionsPicker) => {
  const options = {
    ...defaultOptions,
    ...optionsPicker,
  };
  ...
  return new Promise(async (resolve, reject) => {
    try {
      const response = await VpsReactNativeImagePicker.openImagePicker(options);
      if (response?.length) {
        if (isSingle) {
          resolve(response[0]);
        }
...
export const openCamera = (optionsPicker) => {
  const options = {
    ...defaultOptions,
    ...optionsPicker,
  };
  ...
  return new Promise(async (resolve, reject) => {
    try {
      const response = await VpsReactNativeImagePicker.openCamera(options);
      if (response?.length) {
        if (isSingle) {
          resolve(response[0]);
        }
